...
public class CMDCooldown implements CommandExecutor {
	...
	public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
		...
		try {
			kitUsage.setDate(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2000-1-1 12:00:00"));
		} catch (ParseException e) {
			e.printStackTrace();
			throw new CommandException(Text.of(TextColors.GOLD, e.getMessage(), false));
		}
		
		list.put(kit.getName(), kitUsage);
		player.offer(Keys.KIT_USAGES, list);

		player.sendMessage(Text.of(TextColors.DARK_GREEN, "Reset cooldown of kit ", kit.getName(), " for ", player.getName()));
		src.sendMessage(Text.of(TextColors.DARK_GREEN, "Reset cooldown of kit ", kit.getName(), " for ", player.getName()));
